...
gseafunc <- function(gmtfile, pval_cutoff, fig_title){
    for(res in list(res_CD4Tn, res_CD4Tnn, res_CD8Tn, res_CD8Tnn)){
      # print comparison
      print(sub("group ","",strsplit(attr(res,"elementMetadata")$description[2],": ")[[1]][2]))
    ...
    # plotGseaTable(gmtfile[mainPathways], ranked_genes, fgseaRes, gseaParam = 0.5)
      ...
      # only show genesets with padj<0.25
      enriched_genesets <- as.data.frame(fgseaRes[fgseaRes$padj < pval_cutoff,])
      p <- ggplot(enriched_genesets, aes(reorder(pathway, NES), NES, color=padj)) +
...
## Immunological signature gene sets
...
gseafunc(gmtfile=gmtfile, pval_cutoff=0.001, "GSEA-Immunological signature")
```
...
# gseafunc(gmtfile=gmtfile, pval_cutoff=1, fig_title="Exhaustion up/down")
